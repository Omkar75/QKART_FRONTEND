...
class Checkout extends React.Component {
  ...
  deleteAddress = async (addressId) => {
    ...
    if (this.validateResponse(errored, response, "address deleted")) {
      if (response) {
  ...
  checkout = async () => {
    ...
    if (this.validateResponse(errored, response, "checkout")) {
        ...
        let reduce = this.cartRef.current.calculateTotal()
        let newbalance = localStorage.getItem('balance') - reduce
  ...
  order = async () => {
    ...
    if(localStorage.getItem('balance') < this.cartRef.current.calculateTotal()){
      message.error("You Don't have enough balance")
    }else if(this.state.addresses.length === 0){
      message.error("Address not given")
    }else{
      this.checkout()
    }
  };
  ...
  async componentDidMount() {
    ...
    if(localStorage.getItem('username') && localStorage.getItem('token') !== ''){
      ...
      message.error("User must login first")
      ...
      <>
        ...
        <div className="checkout-container">
          <Row>
            ...
            <Col span={24} xs={{ order: 2 }} sm={{ order: 1 }} md={{ span: 18 }} >
                ...
                <div>
                  ...
                  <Radio.Group value={1}>
                    <Radio style={radioStyle} value={1}>
                      Wallet
                      <strong> (₹{localStorage.getItem('balance')} available)</strong>
                    </Radio>
                  </Radio.Group>
                </div>
            ...
            {/* Display the cart */}
            <Col span={24} xs={{ order: 1 }} sm={{ order: 2 }} md={6} className="checkout-cart">
              <div>
