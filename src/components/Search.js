...
class Search extends React.Component {
  ...
  getProducts = async () => {
    ...
    // here you have to set the state filteredProducts with the products you have fetched. Like this:
    /*
      if (pro !== undefined) {
        this.products = pro;
        const products = [...this.products];
  ...
  search = (text) => {
    ...
    this.setState({
      filteredProducts : this.state.filteredProducts.filter((prod) => {
        return prod.name.toLowerCase().includes(text.toLowerCase()) || prod.category.toLowerCase().includes(text.toLowerCase())
      })
    })
  };
      ...
      <>
        ...
        <Header history={this.props.history}>
          ...
          <Space direction="vertical">
            <Input.Search placeholder="Search" onChange={this.debounceSearch} onSearch={this.search} enterButton style={{ width: 200 }}/>
          </Space>
